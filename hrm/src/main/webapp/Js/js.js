...
function educateValidate(){
	if(educateForm1.name.value==""){
		alert("请添写培训名称！");		
		educateForm1.name.focus();
		return false;
	}
	if(educateForm1.purpose.value==""){
		alert("请添写培训目的！");		
		educateForm1.purpose.focus();
		return false;
	}
	if(educateForm1.begintime.value==""){
		alert("请添写培训开始时间！");		
		educateForm1.begintime.focus();
		return false;
	}
	if(!checkIsValidDate(educateForm1.begintime.value)){
		alert("输入的时间不合法！（年-月-日）34");
		educateForm1.begintime.focus();
		return false;
	}

	if(educateForm1.endtime.value==""){
		alert("请添写培训结束时间！");		
		educateForm1.endtime.focus();
		return false;
	}
	if(!checkIsValidDate(educateForm1.endtime.value)){
		alert("输入的时间不合法！（年-月-日）4");
		educateForm1.endtime.focus();
		return false;
	}
	if(!checkDateEarlier(educateForm1.begintime.value,educateForm1.endtime.value)){
		alert("起始日期不能晚于结束日期！");
		educateForm1.begintime.focus();
		return false;
	}

	if(educateForm1.teacher.value==""){
		alert("请添写培训讲师！");		
		educateForm1.teacher.focus();
		return false;
	}
	if(educateForm1.student.value==""){
		alert("请添写培训人员！");		
		educateForm1.student.focus();
		return false;
	}
	return true;
}
...
function stipendValidate()
{
	...
	else if(stipendForm.basic.value==""){
		alert("请添写基本薪金！");
		stipendForm.basic.focus();
	}
else if(stipendForm.eat.vaule==""){
		alter("请填写饭补！");
		stipendForm.eat.focus();
	}
else if(stipendForm.house.value==""){
	alter("请填写房补");
	stipendForm.basic.focus();
}
else if(stipendForm.duty.value==""){
	alter("请填写全勤奖");
	stipendForm.duty.focus();
}
else if(stipendForm.scot.value==""){
	alter("请填写赋税");
	stipendForm.scot.focus();
}
else if(stipendForm.other.value==""){
	alter("请填写额外补助");
	stipenForm.other.focus();
}else if(stipendForm.punishment.value==""){
	alter("请输入罚款");
}
}